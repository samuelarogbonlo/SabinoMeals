...
import com.sam.sabinomeals.ui.comment.CommentFragment
...
class FoodDetailFragment : Fragment(), TextWatcher {
    ...
    private fun addRatingFood(ratingValue: Float?) {
        FirebaseDatabase.getInstance()
            .getReference(Common.CATEGORY_REFERENCE)        //Select menu in category
            .child(Common.CATEGORY_SELECTED?.menu_id!!)     //select foods array
    ...
    private fun displayAddonList() {
        if (Common.FOOD_SELECTED!!.addon!!.isNotEmpty()) {
            ...
            for (addonModel in Common.FOOD_SELECTED!!.addon!!) {
                ...
                chip.text =
                    StringBuilder(addonModel.name!!).append("Naira ").append(addonModel.price!!)
                        .append(")")
    ...
    private fun showDialogRating() {
        ...
        builder.setMessage("Please fill required information");
        val itemView = LayoutInflater.from(getContext()).inflate(R.layout.layout_rating, null);
        val ratingBar = itemView.findViewById(R.id.rating_bar) as RatingBar
